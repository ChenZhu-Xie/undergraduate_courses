...
% \sidenote[-7.5em][]{
% 	在热平衡的黑体辐射系统中， 光波波矢分立取值的原因， 从量子力学的角度， 是位移与动量的不对易所导致的二者的不确定原理； 
% 	从光学角度， 仍是三个维度上的驻波条件， 但所有用到驻波条件的地方， 单色光波及光子的波长、波矢、动量都得是正的， 
% 	这仍源于数物的那个模型； 以上两个使波矢动量分立取值的原因， 均需配合一卦限的1/8波矢、动量空间， 且其模分立取值， 
% 	才能导出正确的态密度。 \\
% 	\\
% 	\raggedright 而热辐射系统的光波三维波矢分立取值的原因， 还可从量子力学的角度， 认为是三维谐振子在相空间中
% 	封闭轨道的体积不连续变化的最小量所限制； 而统计物理则直接用箱归一化边界条件解释之。 尽管黑体辐射可描述为
% 	腔壁的原子内电磁场随机跃迁， 吸收或发出不同初相且振幅非零的单色平面波的结果， 以至于该过程的边界条件可由
% 	容器边界振幅非零，且相位随机的箱归一化描述， 但我仍认为箱归一化只是一种为得到结果的解释罢了。 以上二者需配合
% 	可取负的波矢和动量， 即全波矢、动量空间， 才能导出正确的态密度。
% 	\raggedright 非周期复色场在非特殊形状、非导体边界下要想展开为单色场， 首先需要周期性延拓， 则边界条件必须选用箱归一化； 
