...
\begin{corollary}[Physics - 实数 \& 复数形式的傅里叶级数]
	\begin{equation}\small
		...
		的加和（即其傅里叶逆变换成立的原因）， 是因对于其中任何一个子波系数， 都存在一个傅里叶正变换，以给出其值（完备性）：
		\begin{align}
			&\left\{ \begin{array}{l}
			...
			&\hphantom{==} {\mathbcal E}_0 = {\mathbcal C}_0 , {\mathbcal E}_i = \frac{{\mathbcal C}_{\left| i \right|}}{2} \left( i \neq 0 \right)
...
\marginnote[-16em]{
	\begin{kaobox}[frametitle=推论 2.1.5 脚注I \  注释]
		行波在空域中存在周期的方向上的正交性，可由每一时刻的不含时基本函数族的正交性给定：
		\begin{equation}
			\begin{split}
