...
\begin{corollary}[Physics - 实数 \& 复数形式的傅里叶级数]
	...
	\begin{itemize}[leftmargin=12pt]
		...
		\item[$ _{}^{\romannumeral1} $] 行波在空域中存在周期的方向上的正交性，可由每一时间断面上，不含时基本函数族的正交性给定：
		\begin{equation}
			\begin{split}
			...
			\end{split}
		\end{equation}
		\item[$ _{}^{\romannumeral2} $] 任何时变复色场都可写为行波场的加和（即其傅里叶逆变换成立的原因）， 是因对于其中任何一个子波系数， 都存在一个傅里叶正变换，以给出其值（完备性）：	
		\begin{align}
			&\left\{ \begin{array}{l}
