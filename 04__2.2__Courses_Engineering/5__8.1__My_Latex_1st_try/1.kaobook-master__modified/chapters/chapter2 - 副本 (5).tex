...
\begin{corollary}[Physics - 实数 \& 复数形式的傅里叶级数]
	\begin{equation}\small
		...
		$ \int_\Lambda  { \left[ \cos \left( {{{\bm k}_i} \cdot \bm{r}} \right) \cos \left( {\omega _i}t \right) + \sin \left( {{{\bm k}_i} \cdot \bm{r}} \right) \sin \left( {\omega _i}t \right) \right] \cdot \left[ \sin \left( {{{\bm k}_j} \cdot \bm{r}} \right) \cos \left( {\omega _j}t \right) - \cos \left( {{{\bm k}_j} \cdot \bm{r}} \right) $
		$ \sin \left( {\omega _j}t \right) \right] \cdot {\mathrm d}{\bm r}} =  $ }
	\end{equation}
