...
kx = 2*pi/(Tx*(mx-1))*(-mx/2:mx/2-1);   %倒空间坐标kx
ky = 2*pi/(Ty*(my-1))*(-my/2:my/2-1);   %倒空间坐标ky
kz = 2*pi/(Tz*(mz-1))*(-mz/2:mz/2-1);   %倒空间坐标kz
...
aim1 = im2double(rgb2gray(imread('p2.png')));  %初始化目标场分布
