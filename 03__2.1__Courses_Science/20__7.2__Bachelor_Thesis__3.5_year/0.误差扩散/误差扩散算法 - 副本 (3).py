...
plt.figure("大图") # 图中图的大底板图
plt.subplot(521), plt.imshow(img, 'gray'), plt.title('img') # 在大图上显示第一个小图
# ----------------------------------- 二维快速傅里叶变换 和 逆变换
H_img = np.fft.ifft2(img)
H_img_p_angle = np.angle(H_img) # H_img_angle 的表达方式与 H_img_p_angle 是一样的，正如 H_img_p 的表达方式与 H_img_angle_p 一样，一样。
H_img_p = np.zeros((H_img.shape[0], H_img.shape[1]), dtype=complex) # 之后要让其储存复数值，所以这里得先给它定义一下类型，不然储存不了复数值，这与接下来的img_complex = np.fft.ifft2(img)其实一样
for i in range(H_img_p_angle.shape[0]):
    for j in range(H_img_p_angle.shape[1]):
        # print(H_img_angle[i, j])
        H_img_p[i, j] = complex(math.cos(H_img_p_angle[i, j]), math.sin(H_img_p_angle[i, j])) # 也只是暂时储存值，将angle相位（是纯角度，是实数）转换为相位部分，即虚数。
H_img_p_reconstruct = np.abs(np.fft.ifft2(H_img_p)) # 真正的傅里叶逆变换
# ----------------------------------- 显示不贴随机相位片的原img的 傅里叶变换后的 只取相位部分的全息图H_img_p的 重建像H_img_p_reconstruct
plt.subplot(523), plt.imshow(H_img_p_angle, 'gray'), plt.title('H_img_p_angle') # 无法显示H_img_p，因为是复数数组，所以只能委曲求全= =
plt.subplot(524), plt.imshow(H_img_p_reconstruct, 'gray'), plt.title('H_img_p_reconstruct')
...
H_p_reconstruct = np.abs(np.fft.ifft2(H_p)) # 真正的傅里叶逆变换
# ----------------------------------- 显示贴了随机相位片后的img的 傅里叶变换后的 只取相位部分的全息图H_p的 重建像H_p_reconstruct
plt.subplot(525), plt.imshow(H_p_angle, 'gray'), plt.title('H_img_random_p_angle') # 无法显示H_p或者说H_img_random_p，因为是复数数组，所以只能委曲求全= =
plt.subplot(526), plt.imshow(H_p_reconstruct, 'gray'), plt.title('H_img_random_p_reconstruct')
...
for i in range(H.shape[0]):
    for j in range(H.shape[1]):
        H_p_ERD[i, j] = H_new_function(i, j) / abs(H_new_function(i, j)) # 将第i+1行第j+1列的 H_p 的 angle 置为 H_new 的 相应i+2行 & j+2列的 angle，并将其模归一；其中/为浮点除法，所以可以保证归一化；
...
H_p_ERD_reconstruct = np.abs(np.fft.ifft2(H_p_ERD))
# ----------------------------------- 显示贴了随机相位片后的img的 傅里叶变换后的 误差扩散后的相位全息图H_p_ERD的 重建像H_p_ERD_reconstruct
plt.subplot(527), plt.imshow(H_p_ERD_angle, 'gray'), plt.title('H_img_random_p_ERD_angle') # 无法显示 H_p_ERD 即 H_img_random_p_ERD，因为是复数数组，所以只能委曲求全= =
plt.subplot(528), plt.imshow(H_p_ERD_reconstruct, 'gray'), plt.title('H_img_random_p_ERD_reconstruct')
# ————————————————————————————————————
# def E_O(i, j):
