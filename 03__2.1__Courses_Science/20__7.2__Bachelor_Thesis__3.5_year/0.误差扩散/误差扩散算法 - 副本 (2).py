...
for i in range(img.shape[0]):
    for j in range(img.shape[1]):
        ramdnum = random.uniform(0, 2*math.pi)
        img_complex[i, j] = img[i, j] * complex(math.cos(ramdnum), math.sin(ramdnum)) # 给物前加个随机相位片
# ----------------------------------- 二维快速傅里叶变换
H = np.fft.fft2(img_complex) # 二维快速傅里叶变换算法得到频率分布，返回一个复数数组；fft2(a, s=None, axes=(-2, -1), norm=None)
Hcopy = np.fft.fft2(img_complex) # 不能写 Hcopy = H，因为这样 Hcopy 与 H 都指向同一块区域和内容
H_img = np.angle(H)
H_iimg = np.fft.ifft2(H_img) # 只是先定义一个与H_img相同尺寸的矩阵，赋什么值不重要
for i in range(H_img.shape[0]):
    for j in range(H_img.shape[1]):
        # print(H_img[i, j])
        H_iimg[i, j] = complex(math.cos(H_img[i, j]), math.sin(H_img[i, j])) # 也只是暂时储存值
H_iimg = np.fft.ifft2(H_iimg) # 真正的傅里叶逆变换
