...
long WaitSetting1=9000,WaitCounter=1;    //理论上的1s1次，但与程序有关
int WaitStorer=0;
...
void main()
{
   uchar key,counthang,lieshu[4],i,countlie,fakehang,tag,locationcorrect,numbercorrect,j,randnum,TAGG,timer,length=4,shift=0,start=0,finish=0,k,counthang65,seconds,steps;//350.h里好像已经定义了TAG了，要用TAGG，倒是LAB还没用...虽然LAB不是个变量。——这个是可以定义在main里的，之前之所以不行，是因为定义在了赋值语句后面，比如它就不能放在诸如RI0=0后面。——毕竟要是局部变量无法定义，那就真的完蛋了。——之前变量互相赋值，就是因为全局变量太占空间了（定义为局部变量后，lieshu[j]还真就不给counthang赋值了），所以要定义为局部变量，我tm终于找对了定义变量应该放的位置了。
   P0=0;
   P1=0;
   RI0=0;
